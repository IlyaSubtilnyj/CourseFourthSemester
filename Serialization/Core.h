...
namespace Core
{
	...
	template<>
	inline float decode(const std::vector<uint8_t>& buffer, int16_t& it)
	{
		float result;
		int32_t inter = decode<int32_t>(buffer, it);
		result = *reinterpret_cast<float*>(&inter);
		return result;
	}

	template<>
	inline double decode(const std::vector<uint8_t>& buffer, int16_t& it)
	{
		double result;
		int64_t inter = decode<int64_t>(buffer, it);
		result = *reinterpret_cast<double*>(&inter);
		return result;
	}
